...
# Install with checks
pip install ".[dev,action,pose]" # Install any new dependencies.
echo "Pip install exit code: $?"
pip show iblvideo
python -c "import iblvideo; print('iblvideo location:', iblvideo.__file__); print('iblvideo import successful')"

# Run with html reporting (for github actions)
...
pytest --html=report.html --self-contained-html --basetemp="/local/$(whoami)/pytest-tmp/$(date '+%Y_%m_%d-%H_%M_%S')" tests/test_motion_energy.py tests/test_pose_lp.py tests/test_segmentation_la.py
