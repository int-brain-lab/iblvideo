...
def align_data(eid, one, trial_idx, probe):    
    ...
    '''
    Downloading and aligneing the full ap files (takes very long, 40 GB large)
    '''
...
def plot_ap_raster_wheel(eid, one, t_start, probe):

    ap_chunk, times_data = get_ap_partial(eid, one, t_start, probe)
    
    plt.figure()
    ax0 = plt.subplot(3,1,1)
    
    obs, chans = ap_chunk.shape 
    
    for i in range(chans)[:30]:#[::30]:
        tplot = ap_chunk[:,range(chans)[i]] - np.mean(ap_chunk[:,range(chans)[i]])
        plt.plot(times_data, tplot +i*90) #i*3)    
 
    ax0.title.set_text('ap_signal')
    ax0.set_xlabel('time [s]')     
    ax0.set_yticklabels([])
    ax0.set_ylabel('every 30th channel')
    #ax0.set_ylabel('first 10 channels')

    ax1 = plt.subplot(3,1,2, sharex = ax0)
    
    seg_length = 3 # data segment length in seconds, starting at t_start 
    
    dataset_types = ['spikes.times', 'spikes.depths','spikes.clusters', 'trials.intervals','wheel.position','wheel.timestamps']   
    ...
    first = find_nearest(times, t_start)
    last = int(first + seg_length/T_BIN)
    #return first, last       
    
    ax1.title.set_text('%s_%s' %(eid, probe))    

    ax1.imshow(R[:, first:last], aspect='auto',
...
#    first_w = find_nearest(wheel['timestamps'], t_start)
#    last_w = find_nearest(wheel['timestamps'], t_start + seg_length)
#    plt.plot(wheel['timestamps'][first_w:last_w],wheel['position'][first_w:last_w], linestyle = '', marker = 'o')    
#    ax2.set_xlabel('Time (s)')
#    ax2.set_ylabel('wheel angle')    
#    
#    
#    plt.tight_layout()
#    plt.show() 
